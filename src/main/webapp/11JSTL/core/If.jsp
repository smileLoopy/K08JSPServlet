...
<body>
	...
	<!--  
	if(numer % 2 ==0) 와 동일한 조건의 if문으로 해당 조건의 결과가 
	result에 저장된다.
	...
	<!--  
	Java에서는 값에 대한 비교 (==) 와 객체값에 대한 비교 equals() 가 다르지만 여기서는 같다. 
	Java에서는 비교연산자 ==과 equals()는 다르지만, EL에서는 eq를 통해 
	값에 대한 비교와 문자열에 대한 비교 모두를 할 수 있다.
	여기서 사용하는 eq는 Java의 compareTo()와 같이 사전순으로 문자열을 비교한다. 
	-->
	<!-- 첫번째 if문은 false의 결과가 나온다. -->
	<c:if test="${string eq 'Java' }" var="result2">
		문자열은 Java입니다. <br />
	</c:if>
	<!-- result2의 값이 false 인데 이걸 not 을 붙이면 true가됨.   
	즉, test="${true}" true인경우는 바로 실행함.-->
	...
	<!-- 
	아이디, 패스워드를 입력했을때만 일치 여부를 판단한 후 loginResult에 결과를
	저장한다. 두번째 if문에서 not을 이용해 else 구문을 만들면 된다.
	-->
	<c:if test="${param.user == 'kosmo' and param.pass eq '1234'}" var="loginResult">
		${param.user } 님 하이루~ <br />
	</c:if>
